...
const DocumentEditor: React.FC<DocumentEditorProps> = ({ setView, onAIChatOpen }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-white font-sans overflow-hidden">
      ...
      <div className="flex-1 flex overflow-hidden bg-white">
        ...
        <div className="flex-1 overflow-y-auto px-8 pb-40 flex flex-col items-center bg-white custom-scrollbar">
          <div className="w-full max-w-[900px] min-h-full animate-in fade-in duration-500">
            {appliedTemplate ? (
              <div 
                ...
                onInput={handleInput}
              >
                {appliedTemplate.name === 'PIIA Agreement' ? PIIA_CONTENT(appliedTemplate.state) : (
                  <div className="py-20">
        ...
        <div className="w-12 bg-white border-l border-gray-100 flex flex-col items-center py-6 gap-6 shrink-0 z-40">
          <SideToolButton icon={<FileText size={20} />} active />
...
const SideToolButton = ({ icon, active = false }: { icon: React.ReactNode, active?: boolean }) => (
  <button className={`p-2 rounded-lg transition-colors shrink-0 ${active ? 'text-gray-900 bg-gray-50' : 'text-gray-400 hover:text-gray-800 hover:bg-gray-50'}`}>
    {icon}
  </button>
);
